module LocationSimulatorRequests
  ...
  def reset_locations_body
    response = JSON.parse(load_fixture("reset_location_simulator_response.json"))
    response.to_json
  end
  ...
  def perform_locations_body
    response = JSON.parse(load_fixture("perform_location_simulator_response.json"))
    response.to_json
  end
end
